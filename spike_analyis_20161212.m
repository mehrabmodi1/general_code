...
%             end
            ...
            if odor_dur == 60
                ...
                sus_pts = (PSTH_curves((8./bin_width + 1):(63./bin_width), :));     
                
                pre_pts_t = nanmean(pre_pts, 1);
                on_pts_t = nanmean(on_pts, 1);
                sus_pts_t = nanmean(sus_pts, 1);
                
                [h, p] = ttest2(pre_pts_t, sus_pts_t);
            ...
            if odor_dur == 60
                ...
                rate_vec = ([pre_rate, on_rate, sus_rate, off_rate]).*sf;   %converting rate to per second
...
xlabel('mean spike rate baseline period (Hz)')
ylabel('mean spike rate onset period (Hz)')
hold off
