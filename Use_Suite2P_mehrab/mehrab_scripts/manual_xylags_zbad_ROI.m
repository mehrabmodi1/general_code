function [lag_mat, bad_trs, done_marking] = manual_xylags_zbad_ROI(dataset_stack, ROI_mat)
...
%checking if only ROI_mat needs to be aligned to trial1 of the dataset i.e. frame1 of the ave stack
if isempty(ROI_mat) == 0
    dataset_stack = dataset_stack(:, :, 1);
    dataset_stack = cat(3, dataset_stack, sum(ROI_mat, 3));
