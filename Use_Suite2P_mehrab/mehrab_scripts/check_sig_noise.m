...
left_bk_vals = mean(frame(:, 1:col_width), 2, 'omitnan');
left_bk_vals = movmean(left_bk_vals, 10);
right_bk_vals = mean(frame(:, (row_length - col_width):row_length), 2, 'omitnan');
...
if c(1, 2) > 0.5 && max(max(col_vals)) < bk_ROI_val
    ...
    noise_mat = repmat(noise_vec, 1, row_length);
    noise_mat = double(noise_mat);
    noise_mat(noise_mat > bk_ROI_val) = bk_ROI_val;
