...
%raw_direc.
    ...
    %specifying imaging rate
    fr_line = master_file{79};
    
    frame_time_factor = num2str(1./frame_time, 2);
    try
        fr_line(1, 31:(31 + length(frame_rate) - 1) ) = frame_rate;
    catch
        keyboard
    end
    master_file{79} = fr_line;      %assigned correct imaging rate
    
    
    %specifying object diameter
    if isempty(findstr(raw_direc, 'axon')) == 1
        cell_dia = '15';
    elseif isempty(findstr(raw_direc, 'axon')) == 0
        cell_dia = '6';
    else
    end
    ...
    fr_line(1, 31:32) = '  ';
    fr_line(1, 31:(31 + length(cell_dia) - 1) ) = cell_dia;
