function [resp_areas, sig_trace_mat, sig_cell_mat, sig_cell_block_mat] = cal_sig_responses_20151020(dataset, dff_data_mat, stim_mat, prot_switch_trials, list_direc, an_trial_window)
...
for odor_n = 1:n_odors
    ...
    odor_trs = find(stim_mat(:, 1) == odor_ni);                       %list of trials on which odor_n was delivered
        
    for block_n = 1:length(prot_switch_trials)
        [block_od_trs] = odor_trial_list_builder(stim_mat, prot_switch_trials, odor_ni, block_n, an_trial_window, 1);
        ...
        for cell_n = 1:n_cells
            ...
            if (n_sig_resps./n_od_trs_nnan) > 0.25                          %criterion as per Glenn's paper
