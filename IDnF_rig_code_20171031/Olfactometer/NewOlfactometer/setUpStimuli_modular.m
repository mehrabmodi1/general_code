...
stimGridFields={'duration','stimLatency','odours','isi','firstDilution','secondDilution', 'n_od_pulses', 'inter_pulse_interval', 'post_od_scan_dur', 'rand_train',...
                        'duration_olf2', 'rel_stimLatency_olf2', 'odours_olf2', 'n_od_pulses_olf2', 'inter_pulse_interval_olf2', 'rand_trains_olf2', 'n_rand_trains_olf2', 'pulse_olf2',};
...
for pp=1:length(params) %this loop is for each parameter, not trials
    ...
    if params.rand_trains == 1
        for duration_n = 1:n_durations
            curr_duration = params.duration(duration_n);
            for rand_train_n = 1:n_rand_trains
               rand_train = rand_train_generator(curr_duration, 8, 0.1, 20);       %generating pulse trains with exp mean 8s, min dur 0.1s, max dur 20s 
    ...
    keyboard   
        for duration_n = 1:n_durations
            ...
            for tr_n = 1:length(curr_trs)
                ...
                param_mat(curr_trs, 10) = {simple_pulse_train};                     %inserting simple train into the appropriate rows of param_mat
