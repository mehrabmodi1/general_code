...
if mid_trial == 0
    ...
    olf_ard = serial('COM13');
    set(olf_ard,'BaudRate',9600);
    fopen(olf_ard);
else
end
...
if mid_trial == 0       %case when this is the first trial of a dataset
    final_trial = 0;
    fprintf(olf_ard, '%s', ['<', 1, '>']);          %waking up olfactometer
    pause(0.5);
...
fprintf(olf_ard, '%s', ['<', num2str(final_trial), '>']);           %specifying whether the olfactometer should go to sleep after this trial
pause(0.05);
fprintf(olf_ard, '%s', ['<', num2str(n_pulses), '>']);              %initial delay after scan trigger onset in ms
pause(0.05);
%loop to communicate each pulse's parameters

for pulse_n = 1:n_pulses
    fprintf(olf_ard, '%s', ['<', num2str(pulse_train(pulse_n, 1) ), '>']);     %odor on time in ms 
    pause(0.05);
end
for pulse_n = 1:n_pulses
    fprintf(olf_ard, '%s', ['<', num2str(pulse_train(pulse_n, 2)), '>']);      %odor_off time in ms
    pause(0.05);
end
for pulse_n = 1:n_pulses    
    fprintf(olf_ard, '%s', ['<', num2str(odor_vec(pulse_n)), '>']);            %odor_n for current pulse
    pause(0.05);
end

%communicating last parameter
fprintf(olf_ard, '%s', ['<', num2str(initial_delay), '>']);             %LED control voltage to be delivered
