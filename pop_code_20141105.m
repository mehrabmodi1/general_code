...
while 1
    ...
    %pooling response data across datasets
    insig_resps = find(sig_response_mat == 0);
    response_mat(insig_resps) = nan;
    for odor_n = 1:n_odors
        trial_list = find(odor_t_list == odor_n);
        if isempty(trial_list) == 1
            continue
        else
        end
        
        resps = reshape(response_mat(:, trial_list), 1, []);
        
        if rem(counter, 2) == 1
            if odor_n == 1
                saved_resps_w1 = [saved_resps_w1, resps];
            elseif odor_n == 2
                saved_resps_w2 = [saved_resps_w2, resps];
            elseif odor_n == 3
                saved_resps_w3 = [saved_resps_w3, resps];                
            elseif odor_n == 4
                saved_resps_w4 = [saved_resps_w4, resps];
            else
            end
        elseif rem(counter, 2) == 0
            if odor_n == 1
                saved_resps_ag1 = [saved_resps_ag1, resps];
            elseif odor_n == 2
                saved_resps_ag2 = [saved_resps_ag2, resps];
            elseif odor_n == 3
                saved_resps_ag3 = [saved_resps_ag3, resps];                
            elseif odor_n == 4
                saved_resps_ag4 = [saved_resps_ag4, resps];
            else
            end
        else
        end
...
a = isinf(saved_resps_ag1);
a = find(a == 1);
saved_resps_ag1(a) = [];

a = isinf(saved_resps_ag2);
a = find(a == 1);
saved_resps_ag2(a) = [];

a = isinf(saved_resps_ag4);
a = find(a == 1);
saved_resps_ag4(a) = [];


%plotting
means = [nanmean(saved_resps_w1), nanmean(saved_resps_w2), nanmean(saved_resps_w3), nanmean(saved_resps_w4);...
    nanmean(saved_resps_ag1),nanmean(saved_resps_ag2),nanmean(saved_resps_ag3), nanmean(saved_resps_ag4)];

ses = [nanstd(saved_resps_w1), nanstd(saved_resps_w2), nanstd(saved_resps_w3), nanstd(saved_resps_w4);...
    nanstd(saved_resps_ag1),nanstd(saved_resps_ag2),nanstd(saved_resps_ag3), nanstd(saved_resps_ag4)];

n_vec = [sqrt(length(saved_resps_w1 - sum(isnan(saved_resps_w1)))), sqrt(length(saved_resps_w2 - sum(isnan(saved_resps_w2)))),...
    sqrt(length(saved_resps_w3 - sum(isnan(saved_resps_w3)))), sqrt(length(saved_resps_w4 - sum(isnan(saved_resps_w4))));...
    sqrt(length(saved_resps_ag1 - sum(isnan(saved_resps_ag1)))), sqrt(length(saved_resps_ag2 - sum(isnan(saved_resps_ag2)))),...
    sqrt(length(saved_resps_ag3 - sum(isnan(saved_resps_ag3)))), sqrt(length(saved_resps_ag4 - sum(isnan(saved_resps_ag4))))];

ses = ses./n_vec;

barweb(means, ses, 1, {'Ringers', 'Agarose'})