...
%This is where protocol-specific stuff comes in. I have specified times since vid onset when pulse1 and pulse2 come on.
pulse_times_all = [{[30, 60]}; {[85, 115]}];  %real pulse-times
...
vel_cutoffs = [5, .5];  %cutoffs in mm/s and s to determine if a fly has stopped or is moving. Higher values are considered tracking errors and replaced with NaNs
t_window_orig = [0, 26]; %[0, 26] THIS IS AN IMPORTANT PARAMETER - change code that references it depending on your experiment and analysis goal.  %in s, manually chosen analysis time window after odor transition valve switch
%r_cutoff = [10, 40]; %[5, 45]   
r_cutoff = [0, 50];     %in mm, the range of distances from center outside which flies are discarded as being too close to the arena center (0 mm) or edge (50 mm).      
