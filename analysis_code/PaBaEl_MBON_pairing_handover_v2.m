...
for list_n = 1:size(dataset_list_paths, 1)
    ...
    all_saved_mean_traces_transition_pre_pp = [];
    all_saved_mean_traces_transition_pre_uu = [];
    all_saved_mean_traces_simple_post = [];
    all_saved_mean_traces_transition_post = [];
    all_saved_mean_traces_transition_post_pp = [];
    all_saved_mean_traces_transition_post_uu = [];
    all_tr1_ims = [];
    all_ROI_mats = [];
    ...
    for dir_n = 1:n_dirs
        ...
        saved_mean_traces_transition_pre_pp = [];
        saved_mean_traces_transition_post_pp = [];
        saved_mean_traces_transition_pre_uu = [];
        saved_mean_traces_transition_post_uu = [];
        ...
        %keeping track of what kind of responses are stored where in data matrices
        resp_labels_s = [];
        resp_labels_tr = [];
        ...
        if isempty(mean_trace_pre) == 0
            ...
            resp_labels_tr = [resp_labels_tr; {'Un-Pr'}];
        else
        end
        ...
        if isempty(mean_trace_pre) == 0
            ...
            saved_mean_traces_transition_post = pad_n_concatenate(saved_mean_traces_transition_post, mean_trace_post, 2, nan);
            single_tr_resps_tr_crfly = pad_n_concatenate(single_tr_resps_tr_crfly, single_tr_resps_tr, 2, nan);
            resp_labels_tr = [resp_labels_tr; {'Pr-Un'}];
        else
        end        
        ...
        if isempty(mean_trace_pre) == 0
            ...
            saved_mean_traces_transition_post = pad_n_concatenate(saved_mean_traces_transition_post, mean_trace_post, 2, nan);
            single_tr_resps_tr_crfly = pad_n_concatenate(single_tr_resps_tr_crfly, single_tr_resps_tr, 2, nan);
            resp_labels_tr = [resp_labels_tr; {'EL-Pr'}];
        else
        end
        ...
        if isempty(mean_trace_pre) == 0
            ...
            saved_mean_traces_transition_post = pad_n_concatenate(saved_mean_traces_transition_post, mean_trace_post, 2, nan);
            single_tr_resps_tr_crfly = pad_n_concatenate(single_tr_resps_tr_crfly, single_tr_resps_tr, 2, nan);
            resp_labels_tr = [resp_labels_tr; {'Pr -EL'}];
        else
        end
        ...
        if isempty(mean_trace_pre) == 0
            ...
            saved_mean_traces_transition_post = pad_n_concatenate(saved_mean_traces_transition_post, mean_trace_post, 2, nan);
            single_tr_resps_tr_crfly = pad_n_concatenate(single_tr_resps_tr_crfly, single_tr_resps_tr, 2, nan);
            resp_labels_tr = [resp_labels_tr; {'Un-EL'}];
        else
        end
        ...
        if isempty(mean_trace_pre) == 0
            ...
            resp_labels_s = [resp_labels_s; {'Pr'}];
        else
        end
        ...
        if isempty(mean_trace_pre) == 0
            ...
            saved_mean_traces_simple_post = pad_n_concatenate(saved_mean_traces_simple_post, mean_trace_post, 2, nan);
            single_tr_resps_s_crfly = pad_n_concatenate(single_tr_resps_s_crfly, single_tr_resps_s, 2, nan);
            resp_labels_s = [resp_labels_s; {'Un'}];
        else
        end
        ...
        if isempty(mean_trace_pre) == 0
            ...
            saved_mean_traces_simple_post = pad_n_concatenate(saved_mean_traces_simple_post, mean_trace_post, 2, nan);
            single_tr_resps_s_crfly = pad_n_concatenate(single_tr_resps_s_crfly, single_tr_resps_s, 2, nan);
            resp_labels_s = [resp_labels_s; {'EL'}];
        else
        end
        ...
        if isempty(mean_trace_pre) == 0
            saved_mean_traces_transition_pre_pp = pad_n_concatenate(saved_mean_traces_transition_pre_pp, mean_trace_pre, 2, nan);
            saved_mean_traces_transition_post_pp = pad_n_concatenate(saved_mean_traces_transition_post_pp, mean_trace_post, 2, nan);
            single_tr_resps_tr_crfly = pad_n_concatenate(single_tr_resps_tr_crfly, single_tr_resps_tr, 2, nan);
            resp_labels_tr = [resp_labels_tr; {'Pr-Pr'}];
            
            figure('Name','paired - paired PID traces')
        ...
        if isempty(mean_trace_pre) == 0
            ...
            saved_mean_traces_transition_post_uu = pad_n_concatenate(saved_mean_traces_transition_post_uu, mean_trace_post, 2, nan);
            single_tr_resps_tr_crfly = pad_n_concatenate(single_tr_resps_tr_crfly, single_tr_resps_tr, 2, nan);
            resp_labels_tr = [resp_labels_tr; {'Un-Un'}];
            
            figure('Name','unpaired - unpaired PID traces')
        ...
        %extra sets of transition traces for the no LED control dataset, where two sets of transition stimuli were delivered
        all_saved_mean_traces_transition_pre_pp = pad_n_concatenate(all_saved_mean_traces_transition_pre_pp, saved_mean_traces_transition_pre_pp, 3, nan);
        all_saved_mean_traces_transition_post_pp = pad_n_concatenate(all_saved_mean_traces_transition_post_pp, saved_mean_traces_transition_post_pp, 3, nan);
        all_saved_mean_traces_transition_pre_uu = pad_n_concatenate(all_saved_mean_traces_transition_pre_uu, saved_mean_traces_transition_pre_uu, 3, nan);
        all_saved_mean_traces_transition_post_uu = pad_n_concatenate(all_saved_mean_traces_transition_post_uu, saved_mean_traces_transition_post_uu, 3, nan);
...
%calling session response trend plotter for transition stimulus trials
fig_n = fig_n + 1;
resp_labels_tr;
color_vecs = [];
for color_n = 1:size(resp_labels_tr, 1)
    curr_label = resp_labels_tr{color_n};
    if strcmp(curr_label((end - 1):end), 'Pr') == 1
        color_vecs = [color_vecs; paired_color];
    elseif strcmp(curr_label((end - 1):end), 'Un') == 1
        color_vecs = [color_vecs; unpaired_color];
    elseif strcmp(curr_label((end - 1):end), 'EL') == 1
        color_vecs = [color_vecs; EL_color];
    end
end

plot_session_resp_trend(single_tr_resps_tr_all, 5.5, color_vecs, fig_n)
fig_wrapup(fig_n, []);


%calling session response trend plotter for simple stimulus trials
fig_n = fig_n + 1;
color_vecs = [];
for color_n = 1:size(resp_labels_s, 1)
    curr_label = resp_labels_s{color_n};
    if strcmp(curr_label((end - 1):end), 'Pr') == 1
        color_vecs = [color_vecs; paired_color];
    elseif strcmp(curr_label((end - 1):end), 'Un') == 1
        color_vecs = [color_vecs; unpaired_color];
...
plot_session_resp_trend(single_tr_resps_s_all, 5.5, color_vecs, fig_n)
fig_wrapup(fig_n, []);
...
%computed separately for paired-paired and unpaired-unpaired traces for the control dataset
if isempty(all_saved_mean_traces_transition_pre_pp) == 0
    mean_paired_pre_t_pp = mean(squeeze(all_saved_mean_traces_transition_pre_pp(:, 1, :)), 2, 'omitnan');
    se_paired_pre_t_pp = std(squeeze(all_saved_mean_traces_transition_pre_pp(:, 1, :)), [], 2, 'omitnan')./sqrt(size(all_saved_mean_traces_transition_pre_pp, 3));
    mean_unpaired_pre_t_uu = mean(squeeze(all_saved_mean_traces_transition_pre_uu(:, 1, :)), 2, 'omitnan');
    se_unpaired_pre_t_uu = std(squeeze(all_saved_mean_traces_transition_pre_uu(:, 1, :)), [], 2, 'omitnan')./sqrt(size(all_saved_mean_traces_transition_pre_uu, 3));
    max_y = max([max_y; max((mean_paired_pre_t_pp + se_paired_pre_t_pp))]);
    max_y = max([max_y; max((mean_unpaired_pre_t_uu + se_unpaired_pre_t_uu))]);

    mean_paired_post_t_pp = mean(squeeze(all_saved_mean_traces_transition_post_pp(:, 1, :)), 2, 'omitnan');
    se_paired_post_t_pp = std(squeeze(all_saved_mean_traces_transition_post_pp(:, 1, :)), [], 2, 'omitnan')./sqrt(size(all_saved_mean_traces_transition_post_pp, 3));
    mean_unpaired_post_t_uu = mean(squeeze(all_saved_mean_traces_transition_post_uu(:, 1, :)), 2, 'omitnan');
    se_unpaired_post_t_uu = std(squeeze(all_saved_mean_traces_transition_post_uu(:, 1, :)), [], 2, 'omitnan')./sqrt(size(all_saved_mean_traces_transition_post_uu, 3));
    max_y = max([max_y; max((mean_paired_post_t_pp + se_paired_post_t_pp))]);
    max_y = max([max_y; max((mean_unpaired_post_t_uu + se_unpaired_post_t_uu))]);
...
%adding paired-paired and unpaired-unpaired transition trace plots, in the case of the no LED control dataset
if isempty(mean_unpaired_post_t_uu) == 0
    fig_n = fig_n + 1;
    figure('Name','Transition responses, paired-paired od.')
    shadedErrorBar([], mean_paired_pre_t_pp, se_paired_pre_t_pp, {'Color', [0.65, 0.65, 0.65], 'lineWidth', 2}, 1);
    hold on
    shadedErrorBar([], mean_paired_post_t_pp, se_paired_post_t_pp, {'Color', [0, 0, 0], 'lineWidth', 2}, 1);
    ax_vals = axis;
    ax_vals(4) = max_y.*1.2;
    axis(ax_vals);
    ylabel('dF/F response');
    set_xlabels_time(fig_n, frame_time, 10);
    fig_wrapup(fig_n, []);
    od_color = [paired_color; paired_color];
    add_stim_bar(fig_n, stim_frs_trans, od_color);


    fig_n = fig_n + 1;
    figure('Name','Transition responses, unpaired-unpaired od.')
    shadedErrorBar([], mean_unpaired_pre_t_uu, se_unpaired_pre_t_uu, {'Color', [0.65, 0.65, 0.65], 'lineWidth', 2}, 1);
    hold on
    shadedErrorBar([], mean_unpaired_post_t_uu, se_unpaired_post_t_uu, {'Color', [0, 0, 0], 'lineWidth', 2}, 1);
    ax_vals = axis;
    ax_vals(4) = max_y.*1.2;
    axis(ax_vals);
    ylabel('dF/F response');
    set_xlabels_time(fig_n, frame_time, 10);
    fig_wrapup(fig_n, []);
    od_color = [unpaired_color; unpaired_color];
    add_stim_bar(fig_n, stim_frs_trans, od_color);
else
end
...
%---------------------------------
%worker functions
function [mean_trace_pre, mean_trace_post, single_tr_resps_tr, PID_traces] = plot_hover_traces(olf1_odn, olf2_odn, od2_name, od1_color, od2_color, fign,...
    stim_mat, stim_mat_simple, od_col_ns, dur_col_ns, dff_data_mat_f, pairing_tr, y_ax_lim, plot_means, suppress_plots, n_sec, direc)
frame_time = 0.099;
