function vout = tsmovavg_m(vin, mode, varargin)
...
if nargin == 4
    ...
    elseif dim == 2
        ...
        error(message('finance:ftseries:tsmovavg:InvalidDIM'));
    end

elseif (nargin < 3) || (nargin > 4)
    error(message('finance:ftseries:tsmovavg:InvalidNumOfInputs'));
end
...
switch lower(mode(1))
    ...
    case 'e' % Exponential
        ...
        elseif nargin <= 4
            ...
            error(message('finance:ftseries:tsmovavg:InvalidNumOfInputsExponential'));
        end
        
        if numel(timePer) ~= 1 || ((timePer <= 0) || (timePer >= observ))
           error(message('finance:ftseries:tsmovavg:InvalidLag'));
        end
    ...
    case 't' % Triangular
        ...
        elseif nargin <= 4
            ...
            error(message('finance:ftseries:tsmovavg:InvalidNumOfInputsTriangular'));
        end
    ...
    case 'w' % Weighted
        ...
        if (nargin >= 3) && (nargin <= 4)
            ...
            if wi ~= 1
                error(message('finance:ftseries:tsmovavg:WeightMustBeRowVector'));
            end

        else
            error(message('finance:ftseries:tsmovavg:InvalidNumOfInputsWeighted'));
        end
    ...
    otherwise
        error(message('finance:ftseries:tsmovavg:InvalidMode'));
end
...
if numel(lag) ~= 1 || ((lag <= 0) || (lag >= observ))
    error(message('finance:ftseries:simplema:InvalidLag'));
end
