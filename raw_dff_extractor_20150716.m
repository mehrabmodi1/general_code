...
list_direc = ['C:\Data\CSHL\dataset_list_glc_ara_20150917.txt'];
...
while 1
    ...
    try 
        del = dataset(1).process.good_trial;                    %checking if bad trials have already been identified
        ...
        bad_cell_list = bad_cell_catcher(dataset, direc, good_trials);
    ...
    %try
        switch separate_sets
            ...
            case 'Track through'
                dataset_ROIs = selectROIs(dataset_ROIs, 'soma');               %calling Rob's ROI-drawing function
        end
    %catch
    %    keyboard
    %end
    ...
    for trial_n = 1:size(dataset, 2)
        ...
        if trial_n < size(dataset_ROIs, 2)
           ROI_mat = dataset_ROIs(trial_n).ROI(2).roi;
