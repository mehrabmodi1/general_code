...
%added by Mehrab Modi 20200909
if isempty(find(strcmp(varargin,'width_multiplier')))==0
    width_multiplier = varargin{find(strcmp(varargin,'width_multiplier'))+1};
end
%%
if size(fc,1)==1
...
for i=1:size(Y,2)
    ...
    f=f/max(f)*0.3*width_multiplier; %normalize
    F(:,i)=f;
    U(:,i)=u;
    MED(:,i)=nanmedian(Y{i});
    MX(:,i)=nanmean(Y{i});
...
elseif setX == 1
    ...
    elseif length(x) == 1
        axis([(x - 0.5*x), (x + 0.5*x), min(U(:)), max(U(:))]);
    end
end
%% Set x-labels
xL2={''};
