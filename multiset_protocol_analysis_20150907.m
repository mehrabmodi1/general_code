clear all
close all
%list_direc = ['D:\Data\CSHL\dataset_list_Toshi_KC_DN_Led_20150708.txt']; %expt datasets
%list_direc = ['D:\Data\CSHL\dataset_KC_DN_pairing_control_20150828.txt']; %control datasets  
list_direc = ['D:\Data\CSHL\dataset_list_glc_ara_20150917.txt']; %expt datasets
...
while 1
    ...
    bad_tr_list = find(del == 1);
    
    %making sure that at least half the trials in each block were acquired
    
    
    
    %getting rid of manually identified cells from set of significant
    %responder cells and thus from subsequent analysis
%     for odor_ni = 1:length(odor_list)
%         odor_n = odor_list(odor_ni);
%         bad_cell_list = bad_cell_list_manual{direc_counter, odor_ni};
%         sig_cell_list = find(sig_cell_mat(:, odor_n) == 1);       %list of indices of significant cells in sig_cell_mat
%         bad_cell_indices = sig_cell_list(bad_cell_list);          %list of indices of cells to be de-significant-ified
%         sig_cell_mat(bad_cell_indices, :) = 0;
%         sig_cell_block_mati(bad_cell_indices, :, :) = 0;
%         n_cellsi = n_cells - length(bad_cell_list);
%     end
    
%     n_cells_saved = [n_cells_saved; n_cellsi];
...
for odor_n = 1:length(odor_list)
    ...
    errorbar([1:length(mean_vec)], mean_vec, se_vec, 'Color', curr_plot_color, 'LineWidth', line_widths)
    hold on
    ...
    %checking if dataset type is control; and plotting responses pre vs
    %post for both odors pooled.
    if set_type == 0 
        mean_vec_first_pooled = [mean_vec_first_pooled; mean_vec_first];
        mean_vec_last_pooled = [mean_vec_last_pooled; mean_vec_last];
        se_vec_first_pooled = [se_vec_first_pooled; se_vec_first];
        se_vec_last_pooled = [se_vec_last_pooled; se_vec_last];
        
        if odor_n == 2
            grey = [0.75, 0.75, 0.75];
            fig_handle9 = figure(9);
            errorbar(mean_vec_first_pooled, mean_vec_last_pooled, se_vec_first_pooled, 'O', 'MarkerFaceColor', grey, 'MarkerEdgeColor', grey, 'MarkerSize', marker_sizes, 'Color', grey)
            hold on
            h = herrorbar(mean_vec_first_pooled, mean_vec_last_pooled, se_vec_last_pooled, ['.' grey])
            set(h, 'Color', curr_plot_color)
            xlabel('Single cell responses in first block')
            ylabel('Single cell responses in last block')
            %set(gcf, 'Color ', 'w')
            ...
            linex = [1:1:max([mean_vec_first; mean_vec_last])];
            liney = polyval(line_pars, linex);
            plot(linex, liney, '-', 'Color', grey, 'LineWidth', line_widths./2)
            ...
            axis([del_min, del_max, del_min, del_max])
            plot([del_min, del_max], [del_min, del_max], '--', 'Color', [0.60, 0.60, 0.60])
            set(fig_handle9, 'Position', [100, 100, plot_height, plot_width]);
            set(gca,'FontSize', axis_font_size, 'ticklength', tick_length, 'linewidth', box_l_width)
...
[odor_hist(:, 1), hist_centers(:, 1)] = hist(odor1_normdiff, n_bins);
[odor_hist(:, 2), hist_centers(:, 2)] = hist(odor2_normdiff, n_bins);
...
for odor_n = 1:n_odors
    plot(hist_centers(:, odor_n), odor_hist(:, odor_n), 'Color', color_vec(odor_n, :), 'LineWidth', line_widths)      
    hold on
   
end

%adding control data to the plot and doing an ANOVA
if set_type == 0
    save('D:\Data\CSHL\Analysed_data\file_dumps\miltiset_protocol_analysis_20150829.mat', 'data_to_save')
    save('D:\Data\CSHL\Analysed_data\file_dumps\miltiset_protocol_analysis_20150829_ses.mat', 'data_to_save_ses')
elseif set_type == 1
    fig_handle3 = figure(3)
    del = load('D:\Data\CSHL\Analysed_data\file_dumps\miltiset_protocol_analysis_20150829.mat');
    del = del.data_to_save;
    odor_c_data = del{1, 1};
    odor_c_data = [odor_c_data; del{1, 2}];
    
    c_data_diff = odor_c_data(:, 2) - odor_c_data(:, 1);
    [odor_hist_c(:, 1), hist_centers_c(:, 1)] = hist(c_data_diff, n_bins);
    odor_hist_c(:, 1) = odor_hist(:, 1)./sum(odor_hist(:, 1)).*100;           %normalising numbers of cells in each bin
    plot(hist_centers_c(:, 1), odor_hist_c(:, 1), 'Color', [0.65, 0.65, 0.65], 'LineWidth', line_widths)       
...
X_mat = pad_vectors([{odor1_normdiff}, {odor2_normdiff}, {c_data_diff}], 1);
[panova, del, stats] = anova1(X_mat);
[c] = multcompare(stats, 'ctype', 'hsd', 'alpha', 0.01);





%Adding control cell responses to scatter plot of pre vs post response sizes
if set_type == 1
    del = load('D:\Data\CSHL\Analysed_data\file_dumps\miltiset_protocol_analysis_20150829_ses.mat');
    del = del.data_to_save_ses;
    odor_c_ses = del{1, 1};
    odor_c_ses = [odor_c_ses; del{1, 2}];
    grey = [0.65, 0.65, 0.65];
    fig_handle2 = figure(2);
    errorbar(odor_c_data(:, 1), odor_c_data(:, 2), odor_c_ses(:, 2), 'O', 'MarkerFaceColor', grey, 'MarkerEdgeColor', grey, 'MarkerSize', marker_sizes, 'Color', grey)
    hold on
    h = herrorbar(odor_c_data(:, 1), odor_c_data(:, 2), odor_c_ses(:, 1), ['.' grey])
    set(h, 'Color', grey)
    xlabel('Single cell responses in first block')
    ylabel('Single cell responses in last block')
    %set(gcf, 'Color ', 'w')
    ...
    %paired ttests to check if slope of pre vs post plot is unity
    disp('doing paired ttests to check for unity slopes')
    [h_od1, p_od1] = ttest(odor1_norm(:, 1), odor1_norm(:, 2))
    [h_od2, p_od2] = ttest(odor2_norm(:, 1), odor2_norm(:, 2))
    [h_c, p_c] = ttest(odor_c_data(:, 1), odor_c_data(:, 2))
    
    
    %multiple KS tests to compare odor1, odor2 and control changes in
    %response size
    odor1_diffs = odor1_norm(:, 1) - odor1_norm(:, 2);
    odor2_diffs = odor2_norm(:, 1) - odor2_norm(:, 2);
    control_diffs = odor_c_data(:, 1) - odor_c_data(:, 2);
    
    disp('doing multiple KS tests to compare pairs of response changes')
    [h_od1_od2, p_od1_od2] = kstest2(odor1_diffs(:, 1), odor2_diffs(:, 1))
    [h_od1_c, p_od1_c] = kstest2(odor1_diffs(:, 1), control_diffs(:, 1))
    [h_c_od2, p_c_od2] = kstest2(control_diffs(:, 1), odor2_diffs(:, 1))
    
    
    %fitting lines to responses in block1 v/s last block
    line_pars = polyfit(odor_c_data(:, 1), odor_c_data(:, 2), 1);
    linex = [1:1:max([mean_vec_first; mean_vec_last])];
    liney = polyval(line_pars, linex);
    plot(linex, liney, '-', 'Color', grey, 'LineWidth', line_widths./2)
else
