...
while 1
    ...
    if plot_summary_an_only == 0
        ...
        for odor_ni = 1:n_odors
            ...
            odor_name = dataset(1).stim.odourNames(odor_n).odour;
            subplot(ceil(sqrt(n_odors)), ceil(sqrt(n_odors)), odor_ni)
            imagesc(resp_areas(sig_cell_list_temp, odor_trs), [0, 5])
        ...
        %plotting response areas across only analysed trials, for all cells 
        figure(10)
        tr_list = [3:10, 13:20];
        for odor_ni = 1:n_odors
            odor_n = odor_list(odor_ni);
...
%         end
%         
%         errorbar(mean_vec_first, mean_vec_last, se_vec_last, 'O', 'MarkerFaceColor', curr_plot_color, 'MarkerEdgeColor', curr_plot_color, 'MarkerSize', 3, 'Color', curr_plot_color)
%         hold on
...
%             errorbar(mean_vec_first_insig, mean_vec_last_insig, se_vec_last_insig, 'O', 'MarkerFaceColor', curr_plot_color, 'MarkerEdgeColor', curr_plot_color, 'MarkerSize', 3, 'Color', curr_plot_color)
%             hold on
...
%             errorbar(mean_vec_first_pooled, mean_vec_last_pooled, se_vec_first_pooled, 'O', 'MarkerFaceColor', grey, 'MarkerEdgeColor', grey, 'MarkerSize', 3, 'Color', grey)
%             hold on
...
%             linex = [1:1:max([mean_vec_first; mean_vec_last])];
%             liney = polyval(line_pars, linex);
%             plot(linex, liney, '-', 'Color', grey, 'LineWidth', 2)
% 
%             %plotting slope - 1 reference line for figure 2
%             axisi = axis;
%             del_max = max(axisi);
...
% if set_type == 0
%     save('C:\Data\CSHL\Analysed_data\file_dumps\miltiset_protocol_analysis_20150829.mat', 'data_to_save')
%     save('C:\Data\CSHL\Analysed_data\file_dumps\miltiset_protocol_analysis_20150829_ses.mat', 'data_to_save_ses')
% elseif set_type == 1
%     del = load('C:\Data\CSHL\Analysed_data\file_dumps\miltiset_protocol_analysis_20150829.mat');
...
%     plot(hist_centers_c(:, 1), odor_hist_c(:, 1), 'Color', [0.65, 0.65, 0.65], 'LineWidth', 3)       
%        
% end
% 
...
% if set_type == 1
%     del = load('C:\Data\CSHL\Analysed_data\file_dumps\miltiset_protocol_analysis_20150829_ses.mat');
%     del = del.data_to_save_ses;
%     odor_c_ses = del{1, 1};
%     odor_c_ses = [odor_c_ses; del{1, 2}];
%     grey = [0.65, 0.65, 0.65];
%     fig_handle2 = figure(2);
%     errorbar(odor_c_data(:, 1), odor_c_data(:, 2), odor_c_ses(:, 2), 'O', 'MarkerFaceColor', grey, 'MarkerEdgeColor', grey, 'MarkerSize', 3, 'Color', grey)
%     hold on
