...
for direc_list_n = 1:n_direc_lists
    ...
    %     end
        ...
        %using new function for new criterion to define significant
        %responders
        %[sig_cell_mat, sig_cell_1s_mat] = find_long_stim_responders_20160622(dff_data_mat, stim_mat);
    ...
    means_long = mean(saved_resp_vecs);
    means_1s = mean(saved_resp_vecs_1s);

    ses_long = std(saved_resp_vecs)./sqrt(size(saved_resp_vecs, 1));
    ses_1s = std(saved_resp_vecs_1s)./sqrt(size(saved_resp_vecs_1s, 1));
