function [saved_warping_landmarks, ROI_mat_warped] = pick_matched_landmarks(save_path)
...
% clear all
% close all
% save_path = 'C:\Data\Data\Analysed_data\Suite2P_results\20190822\fly2_c739_PABAEL\1\';
reg_stack = load([save_path, 'tr_avg_stack.mat']);
...
while trial_ni < (length(trn_15) - 1)
    if exist([save_path, '\saved_warping_landmarks.mat']) == 2
        saved_warping_landmarks = load([save_path, '\saved_warping_landmarks.mat']);
    ...
    save([save_path, '\saved_warping_landmarks.mat'], 'saved_warping_landmarks');
...
%warping ROI_mat to match landmark pairs specified.
disp('Warping ROIs...')
if exist([save_path, 'ROI_mat_warped.mat']) == 2
    ROI_mat_warped = load([save_path, 'ROI_mat_warped.mat']);
    ROI_mat_warped = ROI_mat_warped.ROI_mat_warped;
else
