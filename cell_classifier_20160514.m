...
r_vecs_saved = zeros(n_odors, 5) + nan;
h_vecs_saved = zeros(n_odors, 5) + nan;
...
for odor_n = 1:n_odors
    ...
    for odor_dur_n = 1:n_odor_durs
        ...
        if  (on_width + sus_width + ramp_width) < 1.2       %three 400 ms Ca decays
            disp('Windows too short for meaningful comparison, skipping')
            h_vec = zeros(1, 5) + nan;
            result_vec = h_vec;
            %keeping track of h_vec across odor durations
            h_vec_mat = [h_vec_mat; h_vec];
        ...
        result_vec = [on_diff, ramp_diff, sus_diff, off_diff, r_multi_pk].*nanmax(h_vec);         %vector of effect sizes, non zero only if significantly different
...
%         %fixing some common discrepancies automatically
%         c_diff_orig = nanmean(h_vec_mat, 1) - h_vec_mat(n_h_vec, :);
%         %1. spuriously detected peaky cell in long duration stim
%         if c_diff_orig(1, 5) ~= 0
%             if h_vec_mat(n_odor_durs, 5) > nanmean(h_vec_mat(:, 5))
%                 non_pk_vec = find(h_vec_mat(:, 5) == 0);
%                 h_vec = h_vec_mat(non_pk_vec, :);
%                 m_diff = 0;
%             else
%                 
%             end
...
%         end
%         
%         %2. Spuriously detected ramping cell in short duration stim
%         if c_diff_orig(1, 2) ~= 0
%             if h_vec_mat(n_odor_durs, 2) == 0
...
%         end
    ...
    try
        r_vec = r_vec_mat(size(r_vec_mat, 2), :);       %taking response sizes for the longest duration stimulus
