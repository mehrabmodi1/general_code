...
function [allclusters, centroidcorr, dendmem]=meta_k_means_tank(eventsmat, distmetric, corrcut, thr, no_iters)
...
dendevents=nansum(eventsmat');
%silentdends=find(dendevents<=1);
silentdends=[];                         %changed on 14/1/13, as dF/F traces are used here and not event counts
...
if length(silentdends)>=numden-4;
    ...
    for k = 3
        ...
        if ~isempty(IDX);
            ...
            % vector of average point to centroid distances for each
            % cluster5
            numvect = [];
            ...
            centroidcorr = corrcoef(centroidmat', 'rows', 'pairwise');
