...
%             cal_sig_responses_20161024(dataset, dff_data_mat, stim_mat, list_direc);
        ...
        %Assigning ROIs to groups based on which ones look like they are on
        %the same axon
        ave_im = Suite2P_results.ops.mimg1;
        ...
        if exist([direc, 'ROI_gps.mat']) ~= 2
            ...
            roi_groups = load([direc, 'ROI_gps.mat']);
            roi_groups = roi_groups.roi_groups;
        end
        
        n_groups = size(roi_groups, 1);
        
        n_syns = size(dff_data_mat, 2);
        for odor_n = 1:n_odors
            odor_ni = odor_list(odor_n);
            od_trs = find(stim_mat_simple(:, 2) == odor_ni);
            ...
            for group_n = 1:n_groups
                ...
                for dur_n = 1:n_od_durs
                    ...
                    curr_mat = mean(resp_mat(:, curr_syns, :), 3, 'omitnan');
                    corr_mat = corrcoef(curr_mat);
                    figure(1)
                    imagesc(corr_mat)
                    colormap(greymap)
                    disp(['single trial corrs for trial-averaged trace', ' group ' int2str(group_n)])
